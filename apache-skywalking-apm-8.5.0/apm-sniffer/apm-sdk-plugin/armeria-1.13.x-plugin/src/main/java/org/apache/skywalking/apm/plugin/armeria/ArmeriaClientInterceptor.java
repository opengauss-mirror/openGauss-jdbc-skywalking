...
public abstract class ArmeriaClientInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    protected void beforeMethod(final URI uri, final HttpMethod httpMethod, final String path) {
        ...
        //add Header info
        SafeCloseable safeCloseable = Clients.withHeaders(
                headers -> {
                    for (CarrierItem item = contextCarrier.items(); item.hasNext(); ) {
                        item = item.next();
                        headers.set(AsciiString.of(item.getHeadKey()), item.getHeadValue());
                    }
                });
        ContextManager.getRuntimeContext().put(KEY_SAFE_CLOSEABLE, safeCloseable);
    }
    ...
    @Override
    public void handleMethodException(final EnhancedInstance objInst, final Method method,
