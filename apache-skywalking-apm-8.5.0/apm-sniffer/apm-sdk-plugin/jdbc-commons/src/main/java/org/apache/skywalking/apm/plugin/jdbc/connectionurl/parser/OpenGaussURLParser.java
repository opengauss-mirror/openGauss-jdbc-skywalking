...
public class OpenGaussURLParser extends AbstractURLParser {
    private static final ILog LOGGER = LogManager.getLogger(OpenGaussURLParser.class);

    private static final int DEFAULT_PORT = 5432;
    private static final String DB_TYPE = "OpenGauss";

    public OpenGaussURLParser(String url) {
        super(url);
    }
    ...
    public ConnectionInfo parse() {
        ...
        if (hostSegment.length > 1) {
            ...
            if (hostAndPort.length != 1) {
                return new ConnectionInfo(ComponentsDefine.OPENGAUSS_DRIVER, DB_TYPE, hostAndPort[0], Integer.valueOf(hostAndPort[1]), fetchDatabaseNameFromURL());
            } else {
                return new ConnectionInfo(ComponentsDefine.OPENGAUSS_DRIVER, DB_TYPE, hostAndPort[0], DEFAULT_PORT, fetchDatabaseNameFromURL());
            }
        }
    }
